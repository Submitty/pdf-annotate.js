...
export default class RemoteUserStoreAdapter extends StoreAdapter {
  constructor(userId = 'user', globalEdit = false) {
    ...
    this.getAnnotation = (documentId, annotationId) => {
      return new Promise(async (resolve, reject) => {
        try {
          return resolve((await getAnnotations(documentId, this._userId))
    ...
    this.deleteAnnotation = (documentId, annotationId) => {
      return new Promise(async (resolve, reject) => {
        let annotation = (await getAllAnnotations(documentId)).filter(i => i.uuid === annotationId)[0] || {};
        if (!annotation) {
    ...
    this.getComments = (documentId, annotationId) => {
      return new Promise(async (resolve, reject) => {
        resolve((await getAnnotations(documentId, this._userId)).filter((i) => {
    ...
    this.deleteComment = (documentId, commentId) => {
      return new Promise(async (resolve, reject) => {
        let comment = (await getAllAnnotations(documentId)).filter(i => i.uuid === commentId)[0] || {};
        if (!comment) {
          return reject('Could not find annotation');
        }
...
export async function getAllAnnotations(documentId) {
  ...
  if (!annotationsFound) {
    ...
    all_annotations.forEach((annotation) => {
      localStorage.setItem(`${documentId}/${annotation.userId}/annotations`, JSON.stringify(Array.isArray(annotation) ? annotation : [annotation]));
    });
  }
...
export async function getAnnotations(documentId, userId, pageNumber) {
  ...
  if (!annotations) {
    let annotationResponse = await axios.get(`${BASE_URL}/program/library/annotations/${documentId}?team_member_id=${userId}`);
    if (annotationResponse.status === 200) {
...
export async function updateAnnotations(documentId, userId, annotations, history) {
  ...
  localStorage.setItem(`${documentId}/${userId}/annotations`, JSON.stringify(Array.isArray(annotations) ? annotations : [annotations]));
