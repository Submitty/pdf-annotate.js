  ...
  constructor() {
    ...
    this.getAnnotation = (documentId, annotationId) => {
      return new Promise(async (resolve, reject) => {
        try {
          return resolve(
            (await getAnnotations(documentId))[await findAnnotation(documentId, annotationId)]
          )
        } catch (err) {
          return reject(err)
        }
    ...
    this.deleteAnnotation = (documentId, annotationId) => {
      return new Promise(async (resolve, reject) => {
        let annotation =
          (await getAnnotations(documentId)).filter((i) => i.uuid === annotationId)[0] || {}
        if (!annotation) {
    ...
    this.getComments = (documentId, annotationId) => {
      return new Promise(async (resolve, reject) => {
        resolve(
          (await getAnnotations(documentId)).filter((i) => {
    ...
    this.deleteComment = (documentId, commentId) => {
      return new Promise(async (resolve, reject) => {
        let comment =
          (await getAnnotations(documentId)).filter((i) => i.uuid === commentId)[0] || {}
        if (!comment) {
          return reject('Could not find annotation')
        }
        let index = -1
...
export async function getAnnotations(documentId, pageNumber) {
  ...
  if (!annotationsFound) {
    ...
    let user_annotations = annotationResponse.data.data.reduce((acc, curr) => {
      ...
      } else acc[curr.team_member_id] = JSON.parse(curr.annotations)
      return acc
    }, {})
